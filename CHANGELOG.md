...
- 🔧 Sync the `bismark/*` modules so Bismark alignment uses intra-instance Bowtie 2/HISAT2 `-p` threading instead of the `--multicore` fork model; Bowtie 2 output is unchanged, but `--aligner bismark_hisat` results shift because HISAT2 is thread-count dependent ([#615](https://github.com/nf-core/methylseq/issues/615))
- ✨ Add opt-in Bismark combined-index alignment via `--combined_index` (directional/PBAT use `--combined_index`, non-directional adds `--combined_index_sequential`). Off by default, so the default alignment output is unchanged. Requires an index built with `bismark_genome_preparation --combined_genome` (built automatically when the pipeline generates the index; not compatible with pre-built/iGenomes indexes). Results are concordance-gated — not byte-identical — versus the classic per-strand model. ([#615](https://github.com/nf-core/methylseq/issues/615))
...
- 🔧 Merge the nf-core/tools v4.0.2 template; drops the Slack/Teams webhook notification feature (`imNotification`/`hook_url`) in line with the template ([#606](https://github.com/nf-core/methylseq/pull/606))
