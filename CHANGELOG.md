...
- 🔧 Add `--mergeContext` ext.arg to `methyldackel/extract` when running with `--comprehensive` param to merge metrics from individual Cytosines in a CpG, producing per-CpG metrics rather than per-Cytosine metrics [#460](https://github.com/nf-core/methylseq/pull/499)
